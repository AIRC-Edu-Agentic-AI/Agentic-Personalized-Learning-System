...
def mock_to_response(mock_dict: dict) -> dict:
    return {
        "id": mock_dict["id"],
        "group_code": mock_dict["group_code"],
        "name": mock_dict["name"],
        "description": mock_dict["description"],
        "created_by": mock_dict["created_by"],
        "members": mock_dict["members"],
        "messages": mock_dict.get("messages", []),
        "resources": mock_dict.get("resources", []),
        "created_at": mock_dict["created_at"],
        "last_active_at": mock_dict.get("last_active_at"),
        "member_count": mock_dict.get("member_count", len(mock_dict["members"]))
    }
